	...
	IosSelect.prototype = {
		...
		initLayer: function () {
			...
			var all_html = [
				...
				'<p class="mask-btns"><a href="javascript:;" class="button-fill done_left sure">确定</a><a href="javascript:;" class="button-fill done_right close">取消</a></p>',
			...
			if (this.options.headerHeight && this.options.itemHeight) {
				...
				this.coverArea1Dom.style.top = this.options.headerHeight + this.options.itemHeight * (this.options.coverArea1Top-1) + this.options.cssUnit;
				this.coverArea2Dom = this.iosSelectLayer.el.querySelector('.cover-area2');
				this.coverArea2Dom.style.top = this.options.headerHeight + this.options.itemHeight * (this.options.coverArea2Top-1) + this.options.cssUnit;
			}
			...
			// 取消 确认 事件
			this.closeBtnDom = this.iosSelectLayer.el.querySelector('.mask-btns .close');
			this.closeBtnDom.addEventListener('click', function (e) {
				self.fallback && self.fallback();
			});

			this.selectBtnDom = this.iosSelectLayer.el.querySelector('.mask-btns .sure');
